...
func normalizeAltmetricsSocialData(ctx context.Context, session *gocql.Session, meta *Meta, ch chan string, errc chan error) error {
	...
	cleanup := func(pk []string, entries []map[string]interface{}) error {
		...
		var set AltmetricsSocialDatas
		for i := range entries {
			set = append(set, AltmetricsSocialData(entries[i]))
		}
...
type AltmetricsSocialDatas []AltmetricsSocialData

func (p AltmetricsSocialDatas) Len() int           { return len(p) }
func (p AltmetricsSocialDatas) Less(i, j int) bool { return p[i].Id() < p[j].Id() }
func (p AltmetricsSocialDatas) Swap(i, j int)      { p[i], p[j] = p[j], p[i] }

type AltmetricsSocialData map[string]interface{}

func (p AltmetricsSocialData) Id() int {
	if v, ok := p["altmetrics_id"]; ok {
		if n, ok := v.(int); ok {
			return n
		}
	}
