...
class IvetlChainedTask(IvetlBaseTask):
    abstract = True

    def run(self, args):

        publisher = args[self.PUBLISHER_ID]
        workfolder = args[IvetlBaseTask.WORK_FOLDER]
...
class IvetlPipeline(IvetlBaseTask):
    abstract = True
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        pts.pipeline_id = self.vizor
        pts.job_id = args[0][IvetlBaseTask.JOB_ID]
        pts.task_id = self.name
        pts.end_time = end_date
        pts.status = self.PL_ERROR
        pts.error_details = str(exc)
