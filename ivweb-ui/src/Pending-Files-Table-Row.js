...
function PendingFilesTableRow (props) {
  ...
  const statusIcon = file.validation_errors
  ...
  const deleteCol = !props.readOnly && !file.validation_errors
    ? (
        <a className="btn btn-default submit-button delete-file-button">
  ...
  const fileStatus = file.validation_errors
...
function PendingFilesTableError (props) {
  ...
  if (file.validation_errors && file.validation_errors.length > 0) {
    errorRows = file.validation_errors.map((item, idx) => {
      return (
        <tr key={`file-${file.file_id}-line-error-${idx}`}>
  ...
  return file.validation_errors
    ? (
      <tr className={`error-list-row file-row-${file.file_id}`}
